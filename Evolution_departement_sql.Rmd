...
dbExecute(conn, "
  CREATE OR REPLACE FUNCTION calcul_iou_multi_rapide(idu text, polygon_avant geometry, avant_table_name text, apres_table_name text)
  RETURNS TABLE (iou_multi numeric, participants_avant text, participants_apres text) AS $$
  DECLARE
      polygon_union geometry := polygon_avant;
      iou_intersect RECORD;
...
dbExecute(conn, "
  ...
  BEGIN
      ...
      IF NOT FOUND THEN
          ...
          -- Insérer les résultats dans le cache
          INSERT INTO multi_calcul_cache (participants_avant, participants_apres, iou_multi, participants_avant_hash)
          VALUES (participants_avant_translate, iou_intersect.idu_translate, iou_intersect.iou_ajust, md5(participants_avant_translate::text));
          
          
          -- Retourner les résultats
      RETURN QUERY SELECT iou_intersect.iou_ajust AS iou, participants_avant_translate AS participants_avant, iou_intersect.idu_translate AS participants_apres;
      ELSE 
...
contour_avant_sql <- st_read(conn, query = "SELECT idu, nom_com, code_com, com_abs, contenance, iou_ajust, idu_translate, iou_multi, participants_avant, participants_apres, geometry FROM contour_avant;")
...
com_abs_avant_sql <- st_read(conn, query = "
                             SELECT avant.idu, avant.nom_com, avant.code_com, avant.com_abs, avant.contenance,                                           avant.idu_avant, avant.geometry, apres.nom_com AS nom_com_apres
