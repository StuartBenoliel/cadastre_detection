...
dbExecute(conn, "TRUNCATE TABLE defusion_com;")
...
dbExecute(conn, "
  ...
  SELECT 
      ajout.idu, ajout.nom_com, ajout.code_com, ajout.com_abs, ajout.contenance,
         avant.idu, ajout.geometry
  FROM
      avant
  JOIN ajout ON ajout.idu = avant.idu_apres;
")
...
dbExecute(conn, paste0("
  ...
  WHERE avant.nom_com IN (SELECT DISTINCT nom_com FROM parc_", num_departement, "_", temps_apres,");"))
...
dbExecute(conn, "
  DELETE FROM fusion_com
  WHERE idu_avant NOT IN 
      (SELECT idu FROM supp
        WHERE idu IN (SELECT idu_avant FROM fusion_com));
")
...
dbExecute(conn, "
  ...
  supp_iou_multi, supp_iou_multi_translate_rapide, max_iou, 
