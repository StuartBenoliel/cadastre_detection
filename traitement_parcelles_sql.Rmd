...
dbGetQuery(conn, paste0(" 
  ...
  FROM 
      pg_indexes 
  WHERE 
      schemaname = '", paste0("cadastre_", num_departement), "'
...
dbExecute(conn, "
  DELETE FROM defusion_com
  WHERE idu_apres NOT IN 
      (SELECT idu FROM ajout
        WHERE idu IN (SELECT idu_apres FROM defusion_com));
")
...
defusion_com_sql <- st_read(conn, query = "SELECT * FROM defusion_com;")
```
...
```{r parcelle changeant de communes, eval=FALSE}
fusion_com_sql <- st_read(conn, query = "SELECT * FROM fusion_com;")
```
...
redecoupage_sql <- st_read(conn, query = "SELECT * FROM redecoupage;")
...
if (nrow(defusion_com_sql) > 0) {
  map <- map + mapview(ins_parc_apres %>%
                       filter(idu %in% defusion_com_sql$idu_apres),  
...
if (nrow(fusion_com_sql) > 0) {
  map <- map + mapview(fusion_com_sql,  
                       layer.name = paste0("Parcelles fusion de communes (état 20",temps_apres,")"), 
                       col.regions = "white",
                       alpha.regions = 0.5, homebutton = F) +
    mapview(ins_parc_avant %>%
              filter(idu %in% fusion_com_sql$idu_avant),  
            layer.name = paste0("Parcelles fusion de communes (état 20",temps_avant,")"), 
            col.regions = "white", alpha.regions = 0.5, homebutton = F)
}
