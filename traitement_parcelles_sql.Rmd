...
dbGetQuery(conn, paste0(" 
  ...
  ORDER BY 
      schemaname, tablename;
           "))
test <- dbGetQuery(conn, "SELECT nom_com, COUNT(*) AS nb_redecoupage
        FROM traitement_24_23_cadastre_60.contour_translation
        GROUP BY nom_com")
```
...
dbExecute(conn, paste0("
  ...
  FROM parc_", num_departement, "_", temps_apres, " apres
  WHERE NOT EXISTS (
    SELECT 1
    FROM parc_", num_departement, "_", temps_avant," avant
...
ins_parc_avant <- st_read(conn, query = paste0("SELECT * FROM ", 
                                               ifelse(indic_bordure, 
                                                      'ins_parc_avant;', 
                                                      paste0("parc_", num_departement, "_", temps_avant, ";"))))

ins_parc_apres <- st_read(conn, query = paste0("SELECT * FROM ", 
                                               ifelse(indic_bordure, 
                                                      'ins_parc_apres;', 
                                                      paste0("parc_", num_departement, "_", temps_apres, ";"))))
```
...
dbExecute(conn, paste0("
  ...
  SELECT apres.idu
  FROM ", ifelse(indic_bordure, 'ins_parc_apres', paste0("parc_", num_departement, "_", temps_apres)), " apres
  WHERE EXISTS (
      SELECT 1
      FROM ", ifelse(indic_bordure, 'ins_parc_avant', paste0("parc_", num_departement, "_", temps_avant)), " avant
      WHERE apres.idu = avant.idu
      AND ST_Equals(
...
modif_avant <- st_read(conn, query = "SELECT * FROM modif_avant_iou_convex;")
...
dbExecute(conn, "TRUNCATE TABLE defusion_com, cas_disparition_commune, chgt_commune;")
...
dbExecute(conn, paste0("
  ...
  FROM (
      ...
      WHERE avant.nom_com NOT IN (SELECT DISTINCT nom_com FROM parc_", num_departement, "_", temps_apres, ")
  ) AS fusion_ajout
  ;
"))
...
dbExecute(conn, paste0("
  ...
  WHERE avant.nom_com IN (SELECT DISTINCT nom_com FROM parc_", num_departement, "_", temps_apres,") OR
...
dbExecute(conn, paste0(" 
  ...
  com_apres AS (
      SELECT DISTINCT
          nom_com, code_com
      FROM 
          parc_", num_departement, "_", temps_apres,"
  )
  SELECT
      df.nom_com_avant,
      df.code_com_avant,
...
dbExecute(conn, paste0("
  ...
  FROM
      avant
  JOIN ajout ON ajout.idu LIKE avant.prefix_idu || '___' || avant.suffix_idu
  WHERE avant.nom_com NOT IN (SELECT DISTINCT nom_com FROM parc_", num_departement, "_", temps_apres,") AND
  avant.nom_com IN (SELECT nom_com_avant FROM cas_disparition_commune GROUP BY nom_com_avant HAVING COUNT(nom_com) = 1);"))
...
dbExecute(conn, paste0(" 
  ...
  com_avant AS (
      SELECT DISTINCT
          nom_com, code_com
      FROM 
          parc_", num_departement, "_", temps_avant,"
  )
  SELECT
      f.nom_com,
      f.code_com,
...
ajout <- st_read(conn, query = "SELECT * FROM ajout_iou_restant;")
supp <- st_read(conn, query = "SELECT * FROM supp_iou_restant;")
...
mapshot(map, url = paste0("parcelles_restantes_",num_departement,"_",temps_apres,"-",temps_avant,".html"))
```
...
bordure <- st_read(conn, query = "SELECT * FROM bordure;")
ins_parc_avant <- st_read(conn, query = paste0("SELECT * FROM parc_", num_departement, "_", temps_avant, ";"))
ins_parc_apres <- st_read(conn, query = paste0("SELECT * FROM parc_", num_departement, "_", temps_apres, ";"))
