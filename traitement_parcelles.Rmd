...
dbExecute(conn, "TRUNCATE TABLE ajout_simp, supp_simp, identique_bis;")
...
X <- dbGetQuery(conn, paste0("
  SELECT * FROM identique_bis WHERE nom_com_avant != nom_com_apres OR code_com_avant != code_com_apres;"))
...
dbExecute(conn, "
  WITH updated_values AS (
    SELECT idu,
           (calcul_iou_intersec(geometry, 'ajout', 0)).*
    FROM supp
  )
  UPDATE supp
  SET iou = updated_values.iou,
...
dbExecute(conn, "
  WITH updated_values AS (
    SELECT idu,
           (calcul_iou_intersec(geometry, 'supp', 0)).*
    FROM ajout
  )
  UPDATE ajout
  SET iou = updated_values.iou,
...
```{r echanges parcelles, eval=FALSE}
translation <- st_read(conn, query = "SELECT * FROM translation;")
X <- dbGetQuery(conn, "
SELECT *
FROM translation
WHERE SUBSTRING(idu_translate FROM 3 FOR 3) <> code_com
")
...
X <- dbGetQuery(conn, "
  ...
  SELECT DISTINCT idu, nom_com, code_com, sub_string, code_com_apres
  FROM filtered")

X <- dbGetQuery(conn,"
  SELECT DISTINCT (echange_parcelles('participants_apres', 'contour_redecoupage')).*") 
...
dbExecute(conn, "
  ...
  supp_simp, identique_bis, fusion_ajout, max_iou, disparition_com, multi_translate_rapide, 
  multi_ajout CASCADE
;")
dbListTables(conn)
```
