...
dbExecute(conn, "
  WITH temp_participants AS (
    SELECT idu, 
        unnest(regexp_split_to_array(participants_avant, ',\\s*')) AS participant
    FROM multi_rapide
  ),
  to_delete AS (
    SELECT DISTINCT t1.idu
    FROM temp_participants t1
    JOIN temp_participants t2
    ON t1.participant = t2.idu AND t1.idu <> t2.idu
  )
  DELETE FROM multi_rapide
  WHERE idu IN (SELECT idu FROM to_delete);
")
...
dbExecute(conn, "TRUNCATE TABLE multi_calcul_cache, echange_parc, echange_parc_possible;")
