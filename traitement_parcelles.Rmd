...
dbExecute(conn, paste0("
  ...
  SELECT DISTINCT ON (red.participants_avant) red.participants_avant, red.nom_com, red.code_com, 
      red.participants_apres, apres.nom_com, ech.code_com_apres
  FROM echange ech
  JOIN redecoupage red ON red.idu = ech.idu_bis
  JOIN parc_", params$num_departement, "_", params$temps_apres, " apres 
  ON apres.code_com = ech.code_com_apres
  LEFT JOIN couples c ON c.nom_avant = red.nom_com AND c.nom_apres = apres.nom_com
...
echange_parc <- dbGetQuery(conn, paste0("WITH chgt_avant AS (
      SELECT nom_com_avant, unnest(regexp_split_to_array(nom_com_avant, ',\\s*')) AS nom_avant
      FROM chgt_com
  ),
  chgt_apres AS (
  ...
  couples AS (
    SELECT a.nom_avant, b.nom_apres
    FROM chgt_avant a
    JOIN chgt_apres b ON a.nom_com_avant = b.nom_com_avant
  ),
  echange AS (
      SELECT 
          (echange_parcelles(
  ...
  SELECT DISTINCT ON (con.participants_avant) con.participants_avant, con.nom_com, con.code_com, 
      con.participants_apres, apres.nom_com, ech.code_com_apres
  FROM echange ech
  JOIN contour_redecoupage con ON con.idu = ech.idu_bis
  JOIN parc_", params$num_departement, "_", params$temps_avant, " apres 
  ON apres.code_com = ech.code_com_apres
  LEFT JOIN couples c ON c.nom_avant = con.nom_com AND c.nom_apres = apres.nom_com
  WHERE (nom_avant IS NULL OR nom_apres IS NULL)"))
...
dbExecute(conn, paste0("
  ...
  SELECT DISTINCT ON (supp.participants_avant) supp.participants_avant, supp.nom_com, supp.code_com, 
      supp.participants_apres, apres.nom_com, ech.code_com_apres
  FROM echange ech
  JOIN supp ON supp.idu = ech.idu_bis
  JOIN parc_", params$num_departement, "_", params$temps_avant, " apres 
  ON apres.code_com = ech.code_com_apres
  LEFT JOIN couples c ON c.nom_avant = supp.nom_com AND c.nom_apres = apres.nom_com
  WHERE (nom_avant IS NULL OR nom_apres IS NULL);")) 
