...
source(file = "database/table_traitement_bis_db.R")
...
cas_disparition_commune <- st_read(conn, query = "SELECT * FROM cas_disparition_commune;")
chgt_commune <- st_read(conn, query = "SELECT * FROM chgt_commune;")
```
...
dbExecute(conn, "TRUNCATE TABLE fusion, subdiv;")
...
dbExecute(conn, "
  INSERT INTO subdiv
  SELECT idu, nom_com, code_com, com_abs, contenance, iou, participants,
      geometry
  FROM supp
...
dbExecute(conn, "
  INSERT INTO contour
  SELECT sup.idu, sup.nom_com, sup.code_com, sup.com_abs, sup.contenance, sup.iou_ajust, 
      sup.idu, ajout.idu, sup.geometry
  FROM supp_iou_multi sup
  INNER JOIN ajout ajout ON ajout.idu_translate = sup.idu
...
dbExecute(conn, "
  ...
  WHERE EXISTS (
      SELECT 1
      FROM contour
      WHERE ajout_iou_translate.idu = contour.idu
  );
")
...
dbExecute(conn, "
  ...
  WHERE EXISTS (
      SELECT 1
      FROM contour
      WHERE supp_iou_multi.idu = contour.idu_translate
  );
")

```

## Cas rapide translation redecoupagermation
...
```{r contour_redecoupage_rapide, eval=FALSE}
contour_redecoupage <- st_read(conn, query = "SELECT * FROM contour_redecoupage;")
```
...
dbExecute(conn, "
  DELETE FROM ajout
  WHERE idu IN (SELECT unnest(regexp_split_to_array(participants_apres_translate, ',\\s*')) FROM contour_redecoupage);
")

dbExecute(conn, "
  DELETE FROM supp
  WHERE idu IN (SELECT unnest(regexp_split_to_array(participants_avant_translate, ',\\s*')) FROM contour_redecoupage);
")
```
...
dbExecute(conn, "
  ...
  supp_simp, max_iou, cas_disparition_commune, multi_translate_rapide, 
  multi_ajout CASCADE
;")
dbListTables(conn)
```
