...
modif_apres <- ins_parc_24 %>% 
  filter(!idu %in% df_geom$idu & !idu %in% ajout$idu) %>% 
  arrange(nom_com)

modif_avant <- ins_parc_23 %>% 
  filter(!idu %in% df_geom$idu & !idu %in% supp$idu) %>% 
  arrange(nom_com)

col_identique <- c()
# Comparer les colonnes en commun
for (col in intersect(names(modif_avant), names(modif_apres))) {
  merged <- full_join(
    select(modif_avant %>% as.data.frame(), idu, !!sym(col)),
    select(modif_apres, idu, !!sym(col)),
    by = "idu",
  ...
  if (all(merged[[paste0(col, "_avant")]] == merged[[paste0(col, "_apres")]], na.rm = TRUE)) {
    col_identique <- c(col_identique, col)
  }
}

modif <- modif_avant %>% as.data.frame() %>% 
  full_join(modif_apres, by = col_identique, suffix = c("_avant", "_apres"))

rm(col_identique, df_geom)
```
...
plot <- ggplot() +
  geom_sf(data = commune, aes(fill = "Communes"),color = "black", alpha = 0.5) +  
  geom_sf(data = bordure, aes(fill = "Bordures étendues"), fill = "lightblue", alpha = 0.5, color = "lightblue") +  
  geom_sf(data = modif_apres, aes(fill = "Parcelles modifiées (2024)"), alpha = 0.5, color = "black", size = 0.01) +
  geom_sf(data = modif_avant, aes(fill = "Parcelles modifiées (2023)"), alpha = 0.5, color = "black", size = 0.01) +
  geom_sf(data = ajout, aes(fill = "Parcelles ajoutées"), alpha = 0.5, color = "black", size = 0.01) +
  geom_sf(data = supp, aes(fill = "Parcelles supprimées"), alpha = 0.5, color = "black", size = 0.01) +
...
map <- mapview(commune, layer.name = "Communes", col.regions = "white", alpha.regions = 0.5) +
  ...
  mapview(modif_apres, layer.name = "Parcelles modifiées (état 2024)", col.regions = "purple", alpha.regions = 0.5) +
  mapview(modif_avant, layer.name = "Parcelles modifiées (état 2023)", col.regions = "pink", alpha.regions = 0.5) +
  mapview(ajout, layer.name = "Parcelles ajoutées", col.regions = "lightgreen", alpha.regions = 0.5) +
  mapview(supp, layer.name = "Parcelles supprimées", col.regions = "red", alpha.regions = 0.5)
...
```{r focus fusion commune}

parc_com_abs_apres <- ajout %>% 
  filter(com_abs != "000") %>%
...
```{r iou ajout/sup}
calcul_iou_for_polygon <- function(polygon, ajout) {
  
  intersecting_geoms <- ajout %>%
  ...
  # Calculer l'IOU
  iou_value <- calcul_iou(combined_geom, polygon)
...
supp <- supp %>%
  rowwise() %>%
  mutate(result = list(calcul_iou_for_polygon(geometry, ajout))) %>%
...
ajout <- ajout %>%
  rowwise() %>%
  mutate(result = list(calcul_iou_for_polygon(geometry, supp))) %>%
  mutate(iou = result$iou,
         participants = result$participants) %>%
  ungroup() %>%
  select(-result)
...
calcul_iou_multi_polygon <- function(polygon, supp, ajout) {
  
  n <- 0
  names_participants_avant <- NULL
  repeat {
    # Trouver les polygones dans supp qui intersectent avec le polygon actuel
    intersecting_geoms_avant <- supp %>%
  ...
  intersecting_geoms_apres <- ajout %>%
    st_filter(st_buffer(polygon, -1), .predicate = st_intersects)
  ...
  # Calculer l'IOU
  iou_value <- calcul_iou(combined_geom_apres, polygon)
...
supp <- supp %>%
  rowwise() %>%
  mutate(result = list(calcul_iou_multi_polygon(geometry, supp, ajout))) %>%
...
calcul_iou_for_polygon_translate <- function(polygon, ajout) {
  ...
  for (i in seq_len(nrow(intersecting_geoms))) {
    ...
    iou_value <- suppressWarnings({calcul_iou(current_geom, polygon)})
  ...
  iou_ajust <- calcul_iou_ajust(geom, polygon)
...
ajout <- ajout %>%
  rowwise() %>%
  mutate(result = list(calcul_iou_for_polygon_translate(geometry, supp))) %>%
  mutate(iou_ajust = result$iou_ajust,
         idu_translate = result$idu_translate) %>%
  ungroup() %>%
  select(-result)
  
translation_apres <- ajout %>% 
  filter(iou_ajust > 0.99) %>% 
...
modif_apres <- modif_apres %>%
  rowwise() %>%
  mutate(result = list(calcul_iou_multi_polygon(geometry, modif_apres, modif_avant))) %>%
  mutate(iou_multi = result$iou,
         participants_avant = result$participants_avant,
         participants_apres= result$participants_apres) %>%
  ungroup() %>%
...
```{r fusion de communes et chgt parcelle}
...
map <- mapview(commune, layer.name = "Communes", col.regions = "white", alpha.regions = 0.5,
  ...
  mapview(ajout_tot %>%
          ...
          homebutton = F) +
  mapview(contour,  layer.name = "Parcelles avant évolution forme (état 2023)", col.regions = "purple", 
  ...
  mapview(ajout, z=c("iou"), layer.name = "Parcelles restantes (état 2024)", alpha.regions = 0.5,
          homebutton = F) +
  mapview(supp, z=c("iou"), layer.name = "Parcelles restantes (état 2023)", alpha.regions = 0.5,
          homebutton = F)
