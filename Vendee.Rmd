...
# Calculer la distance de Hausdorff pour chaque ligne jointe
...
# Choisir un point de référence dans P et Q (par exemple le premier point)
point_ref_P <- st_coordinates(P)[1, 1:2]
point_ref_Q <- st_coordinates(Q)[1, 1:2]

# Calculer la translation nécessaire pour ajuster P à Q
translation <- point_ref_Q - point_ref_P

# Appliquer la translation à toutes les coordonnées de P
P_adjusted <- st_geometry(P) + translation

plot(st_geometry(P_adjusted), col = "blue", main = "Polygone ajusté (P_adjusted)")
plot(st_geometry(Q), col = "red", add = TRUE)

centroid_P <- st_centroid(P)

# Calculer le centroïde de Q
...
# Appliquer la translation à P pour obtenir P_adjusted
P_adjusted <- st_geometry(P) + translation

# Visualisation pour vérifier visuellement
plot(P_adjusted, col = "green", main = "Polygone P ajusté")
