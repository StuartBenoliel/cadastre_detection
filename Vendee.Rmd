...
ajout_tot_sql <- st_read(conn, query = "
  SELECT * 
  FROM ajout_tot;
")

supp_tot_sql <- st_read(conn, query = "
  SELECT * 
  FROM supp_tot;
")
```
...
ins_parc_24_sql <- st_read(conn, query = "
  SELECT DISTINCT
      parc_85_24.*
  FROM
      parc_85_24
...
ins_parc_23_sql <- st_read(conn, query = "
  SELECT DISTINCT
      parc_85_23.*
  FROM
      parc_85_23
...
```{r}
df_diff <- ins_parc_24 %>% 
  filter(!idu %in% ins_parc_24_sql$idu)
...
map <- mapview(commune, 
  ...
  mapview(bordure_sql, 
          layer.name = "Bordures étendues bis", col.regions = "lightgreen", alpha.regions = 0.5) +
  mapview(df_diff, 
          layer.name = "Parcelles pas dans la bordure SQL", col.regions = "red", alpha.regions = 0.5)

# mapshot(map, url = "diff_sql.html")
map
```
...
rm(col_identique, identique, merged)
```
