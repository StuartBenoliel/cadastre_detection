...
plot <- ggplot(na.omit(modif_geom), aes(x = iou_ajust)) +
  ...
  geom_segment(x = quantiles[1], xend = quantiles[1], y = 0, yend = 9, color = "blue", linetype = "dashed", size = 0.6) +
  geom_segment(x = quantiles[2], xend = quantiles[2], y = 0, yend = 12, color = "blue", linetype = "dashed", size = 0.6) +
  geom_segment(x = quantiles[3], xend = quantiles[3], y = 0, yend = 15, color = "blue", linetype = "dashed", size = 0.6) +
  geom_segment(x = quantiles[4], xend = quantiles[4], y = 0, yend = 19, color = "blue", linetype = "dashed", size = 0.6) +
...
map <- mapview(commune, layer.name = "Communes", col.regions = "white", alpha.regions = 0.5) +
  mapview(bordure, layer.name = "Bordures étendues", col.regions = "lightblue", alpha.regions = 0.5) +
  mapview(apres_geom %>% filter(iou_ajust < quantiles[3] | is.na(iou_ajust)),
          z=c("iou_ajust"), layer.name = "Parcelles modifiées (état 2024)", 
          alpha.regions = 0.5) +
  mapview(avant_geom %>% filter(iou_ajust < quantiles[3] | is.na(iou_ajust)), 
          z=c("iou_ajust"), layer.name = "Parcelles modifiées (état 2023)", 
          alpha.regions = 0.5) +
  mapview(ajout_parc, layer.name = "Parcelles ajoutées",col.regions = "purple", alpha.regions = 0.5) +
  mapview(supp_parc, layer.name = "Parcelles supprimées", col.regions = "lightgreen",alpha.regions = 0.5)
