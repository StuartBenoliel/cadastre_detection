...
# Créer les données de polygones multiples
...
# Visualiser les données de base
plot(st_geometry(multipolygon_data), col = c("red", "blue", "green", "orange"), border = "black")

# Fonction pour trouver l'union des polygones touchants et les IDs correspondants
find_union_and_ids <- function(sf_data, input_polygon) {
  intersecting_indices <- which(st_intersects(sf_data, input_polygon, sparse = FALSE))
  if (length(intersecting_indices) == 0) {
    return(list(union_polygon = input_polygon, ids = NA))
  }
  
  intersecting_polygons <- sf_data[intersecting_indices, ]
  union_polygon <- st_union(st_geometry(intersecting_polygons))
  ids <- intersecting_polygons$id
  
  return(list(union_polygon = union_polygon, ids = ids))
}

# Exemple d'utilisation de la fonction avec le premier polygone de multipolygon_data
input_polygon <- multipolygon_data[1, ]
result <- find_union_and_ids(multipolygon_data, st_geometry(input_polygon))
X <- st_union(st_geometry(input_polygon),multipolygon_data)
X <- multipolygon_data %>%
  st_union() %>%
  st_cast("POLYGON") %>%
  st_make_valid()
# Visualiser le polygone d'union résultant
plot(st_geometry(X), col = "grey", border = "black")
plot(result$union_polygon, col = "blue", border = "black")

# Afficher les IDs des polygones participants
print(result$ids)
```
...
dbExecute(conn, "
  ...
  AND EXISTS (
      ...
      AND ST_Equals(ins_parc_apres.geometry, ins_parc_avant.geometry)
  );
")
