...
dbSendQuery(conn, "
  ...
  BEGIN
      ...
      IF NOT ST_IsEmpty(geometry_avant) AND NOT ST_IsEmpty(geometry_apres) THEN
          centroid_apres := ST_Centroid(geometry_apres);
          centroid_avant := ST_Centroid(geometry_avant);
          translation_vector := ST_Translate(geometry_avant, ST_X(centroid_apres) - ST_X(centroid_avant), ST_Y(centroid_apres) - ST_Y(centroid_avant));
        
          -- Calculer l'IoU ajusté avec la géométrie avant ajustée
          geometry_avant_ajust := ST_SetSRID(geometry_avant + translation_vector, ST_SRID(geometry_avant));
...
dbSendQuery(conn, "
  ...
  SELECT * 
  FROM parc_85_24
  WHERE idu NOT IN (SELECT idu FROM parc_85_23);
")
...
dbSendQuery(conn, "
  ...
  SELECT * 
  FROM parc_85_23
  WHERE idu NOT IN (SELECT idu FROM parc_85_24);
")
...
AND EXISTS (
    ...
    AND ST_Equals(p24.geom, ins_parc_23.geom)
);
"
dbSendQuery(conn, query_identique)
...
ORDER BY p24.nom_com;
"
dbSendQuery(conn, query_modif_apres)
...
"
dbSendQuery(conn, query_modif_avant)

query_modif <- "
CREATE TEMP TABLE modif AS
SELECT
    COALESCE(avant.idu, apres.idu) AS idu,
    COALESCE(avant.geometry, apres.geometry) AS geometry,
    COALESCE(avant.contenance, apres.contenance) AS contenance,
    -- Sélectionner toutes les colonnes des deux tables
    avant.*, apres.*
FROM
    modif_avant avant
FULL JOIN
...
WHERE
    -- Filtrer les lignes où les colonnes geometry et contenance sont identiques
    avant.geometry = apres.geometry
    AND avant.contenance = apres.contenance;
"
dbSendQuery(conn, query_modif)
```


```{r modif geometry egal}

identique <- st_read(conn, query = "
   SELECT * FROM identique;
")

modif_apres <- st_read(conn, query = "
  SELECT * FROM modif_apres;
")

modif_avant <- st_read(conn, query = "
  SELECT * FROM modif_avant;
")
...
```{r}
dbSendQuery(conn, "
  CREATE TEMP TABLE modif_calcul AS
  SELECT
      modif.*,
      calcul_iou(modif.geometry_avant, modif.geometry_apres) AS iou,
      calcul_iou_ajust(modif.geometry_avant, modif.geometry_apres) AS iou_ajust
  FROM
      modif;
")
