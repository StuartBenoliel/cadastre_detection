...
```{r fonction sql}
dbExecute(conn, "DROP FUNCTION calcul_iou(geometry,geometry)")
...
dbExecute(conn, "
  ...
  BEGIN
      -- Vérifier si les géométries ne sont pas vides
      IF NOT ST_IsEmpty(geometry_1) AND NOT ST_IsEmpty(geometry_2) THEN
...
dbExecute(conn, "
  CREATE OR REPLACE FUNCTION calcul_iou_multi(idu text, polygon geometry, avant_table_name text, apres_table_name text)
  ...
  BEGIN
      ...
      IF NOT FOUND THEN
          ...
          -- Boucle pour trouver les intersections dans supp_table_name
          LOOP
              -- Mettre à jour polygon_maj avec l'union des géométries de avant_table_name
              EXECUTE '
          ...
          FROM calcul_iou_intersec(polygon_maj, apres_table_name);
...
dbExecute(conn, "
  ...
  BEGIN
      ...
      IF NOT FOUND THEN
          ...
          -- Insérer les résultats dans le cache
          INSERT INTO multi_calcul_cache (participants_avant, participants_apres, iou_multi)
          VALUES (nom_participants_avant, iou_intersect.idu_translate, iou_intersect.iou_ajust);
