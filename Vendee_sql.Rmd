...
dbExecute(conn, "
  ...
  BEGIN
      ...
      IF NOT FOUND THEN
          ...
          VALUES (participants_avant, iou_intersect.participants, iou_intersect.iou);
...
dbExecute(conn, "
  ...
  BEGIN
      ...
      IF NOT FOUND THEN
          ...
          EXECUTE query_sql INTO participants_avant_multi_translate USING polygon_union;
          ...
          -- Insérer les résultats dans le cache
          INSERT INTO multi_calcul_cache (participants_avant, participants_apres, iou_multi)
          VALUES (participants_avant_multi_translate, iou_intersect.idu_translate, iou_intersect.iou_ajust);
          
          
          -- Retourner les résultats
      RETURN QUERY SELECT iou_intersect.iou_ajust AS iou, participants_avant_multi_translate AS participants_avant, iou_intersect.idu_translate AS participants_apres;
      ELSE 
...
contour_avant_sql <- st_read(conn, query = "SELECT * FROM contour_avant;")
...
dbExecute(conn, "
  ...
  WHERE idu IN (SELECT idu FROM subdiv)
    OR iou > 0.99;
")
...
dbExecute(conn, "
  ...
  WHERE idu IN (SELECT idu FROM fusion)
   OR iou > 0.99;
")
...
if (nrow(fusion_sql) > 0) {
  map <- map + mapview(fusion_sql, 
                       ...
                       col.regions = "yellow", alpha.regions = 0.5, homebutton = F) +
    mapview(supp_tot %>%
...
if (nrow(subdiv_sql) > 0) {
  map <- map + mapview(ajout_tot %>%
    ...
    mapview(subdiv_sql,  
            ...
            col.regions = "darkred", alpha.regions = 0.5, homebutton = F)
}
...
mapshot(map, url = "parcelles_iou_85_24-23.html")
